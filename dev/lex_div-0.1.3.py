...
MyClass.i #this works
print(MyClass.f) #but this doesn't (well,not really)

x = MyClass() #here, we define x as an instance of MyClass()
x.i #then we can extract variables
x.f() #and run functions

#the above is essentially what we are doing when we run doc = nlp("some text")

## Test 2: more relevant tests

#this more closely aligns with what we are doing when we run doc = nlp("some text")
class textinfo():
...
bah = textinfo() # with this set up, we HAVE to include an argument (because the __init__ has an argument other than self)

## Test 3: More involved class. This version needs an argument.
class lexdiv1():
...
a = lexdiv1(tokenize("This is a text text"))
a.ttr #this only works because of the last line in class
a.run_ttr("this is a second test test") #this doesn't work

#see test below for working version:

## Test 4: This version allows a user to access the functions OR automatically calculate all indices for a text.
...
b = lexdiv(tokenize("This is a text text"))
b.tokens
b.types
b.ntokens
b.ntypes
...
## Code from the lexical-diversity package:
